...
1. In Cloudflare Dashboard, open **Workers & Pages** and create a Pages project from `jishnuteegala/sound-check-card`.
2. Set production branch to `main`, build command to `pnpm run build`, and build output directory to `dist`.
3. Set the project name to `sound-check-card` and save. Cloudflare uses `wrangler.jsonc` and includes `public/_headers` in the generated `dist` output.
